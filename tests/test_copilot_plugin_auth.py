...
async def test_exchange_request_includes_plugin_headers_and_parses_proxy_endpoint(tmp_path, monkeypatch):
    ...
    assert captured["headers"]["User-Agent"] == "GitHubCopilotChat/0.41.0"
    assert captured["headers"]["Editor-Version"] == "vscode/1.133.0"
    assert captured["headers"]["Editor-Plugin-Version"] == "copilot-chat/0.41.0"

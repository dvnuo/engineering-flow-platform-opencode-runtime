...
@pytest.mark.asyncio
async def test_create_session_supports_parent_id(monkeypatch):
    app = web.Application()
    captured = {}

    async def create(request: web.Request):
        captured["body"] = await request.json()
        return web.json_response({"id": "ses-child"}, status=200)
    ...
    monkeypatch.setenv("EFP_OPENCODE_URL", server_base_url(server))
    out = await OpenCodeClient(Settings.from_env()).create_session(title="Child", parent_id="ses-parent")
    assert out["id"] == "ses-child"
    assert captured["body"] == {"title": "Child", "parentID": "ses-parent"}
    await server.close()


@pytest.mark.asyncio
async def test_message_posts_to_sync_message_endpoint(monkeypatch):
    app = web.Application()
    captured = {}
    ...
    monkeypatch.setenv("EFP_OPENCODE_URL", server_base_url(server))
    out = await OpenCodeClient(Settings.from_env()).message(
        "ses-1",
        {"messageID": "msg_sync_1", "model": "anthropic/claude-sonnet", "parts": [{"type": "text", "text": "hi"}]},
    )
    assert out == {"ok": True}
    assert captured["body"]["messageID"] == "msg_sync_1"
    assert captured["body"]["model"] == {"providerID": "anthropic", "modelID": "claude-sonnet"}
    await server.close()


@pytest.mark.asyncio
async def test_permission_response_preserves_remember(monkeypatch):
    app = web.Application()
    captured = {}

    async def permission(request: web.Request):
        captured["body"] = await request.json()
        return web.Response(status=204)

    app.router.add_post("/session/ses-1/permissions/perm-1", permission)
    server = TestServer(app)
    await server.start_server()

    monkeypatch.setenv("EFP_OPENCODE_URL", server_base_url(server))
    out = await OpenCodeClient(Settings.from_env()).permission_response(
        "ses-1",
        "perm-1",
        {"response": "always", "remember": True},
    )
    assert out == {"success": True}
    assert captured["body"] == {"response": "always", "remember": True}
    await server.close()


@pytest.mark.asyncio
async def test_todo_diff_children_fork_and_mcp_status_methods(monkeypatch):
    app = web.Application()
    calls: list[tuple[str, str]] = []

    async def children(_request: web.Request):
        calls.append(("GET", "/session/ses-1/children"))
        return web.json_response([{"id": "child-1"}])

    async def todo(_request: web.Request):
        calls.append(("GET", "/session/ses-1/todo"))
        return web.json_response([{"text": "item"}])

    async def diff(request: web.Request):
        calls.append(("GET", f"/session/ses-1/diff?{request.query_string}"))
        return web.json_response({"files": []})

    async def fork(request: web.Request):
        calls.append(("POST", "/session/ses-1/fork"))
        assert await request.json() == {"messageID": "msg_prev"}
        return web.json_response({"id": "ses-2"})

    async def mcp(_request: web.Request):
        calls.append(("GET", "/mcp"))
        return web.json_response({"github": {"status": "connected"}})

    app.router.add_get("/session/ses-1/children", children)
    app.router.add_get("/session/ses-1/todo", todo)
    app.router.add_get("/session/ses-1/diff", diff)
    ...
    monkeypatch.setenv("EFP_OPENCODE_URL", server_base_url(server))
    client = OpenCodeClient(Settings.from_env())
    assert await client.children("ses-1") == [{"id": "child-1"}]
    assert await client.todo("ses-1") == [{"text": "item"}]
    assert await client.diff("ses-1", message_id="msg_prev") == {"files": []}
    assert await client.fork("ses-1", "msg_prev") == {"id": "ses-2"}
    assert await client.mcp_status() == {"servers": {"github": {"status": "connected"}}}
    assert calls == [
        ("GET", "/session/ses-1/children"),
        ("GET", "/session/ses-1/todo"),
        ("GET", "/session/ses-1/diff?messageID=msg_prev"),
        ("POST", "/session/ses-1/fork"),
        ("GET", "/mcp"),
    ]
    await server.close()

...
async def test_proxy_forwards_chat_and_responses_to_upstream_with_internal_token(tmp_path, monkeypatch):
    ...
    assert captured[0]["headers"]["User-Agent"] == "GitHubCopilotChat/0.41.0"
    assert captured[0]["headers"]["Editor-Version"] == "vscode/1.133.0"
    assert captured[0]["headers"]["Editor-Plugin-Version"] == "copilot-chat/0.41.0"

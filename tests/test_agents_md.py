...
def test_workspace_agents_md_example_exists():
    template = Path("workspace/AGENTS.md.example")
...
def test_ensure_default_agents_md_uses_workspace_example_when_missing(tmp_path, monkeypatch):
    ...
    generated = ensure_default_agents_md(s).read_text(encoding="utf-8")
    assert generated == Path("workspace/AGENTS.md.example").read_text(encoding="utf-8")
...
def test_agents_md_module_py_compiles():
    import py_compile

    py_compile.compile("efp_opencode_adapter/agents_md.py", doraise=True)

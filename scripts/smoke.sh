...
cat > "${SKILLS_DIR}/smoke_skill/skill.md" <<'EOF'
---
name: smoke_skill
...
EOF
cat > "${TOOLS_DIR}/manifest.yaml" <<'EOF'
tools:
...
(state/'tools-index.json').write_text(json.dumps({'tools':[{'capability_id':'smoke.tool','name':'efp_smoke_tool','opencode_name':'efp_smoke_tool','description':'Smoke read-only tool','enabled':True,'policy_tags':['read_only','smoke'],'runtime_compat':['opencode'],'type':'adapter_action'}]}), encoding='utf-8')
PY

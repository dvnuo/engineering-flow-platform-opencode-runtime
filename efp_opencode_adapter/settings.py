...
class Settings:
    ...
    # Deprecated long-task/recovery knobs retained only so existing env vars do
    # not break settings construction. They are not used by chat execution.
    chat_total_wall_timeout_seconds: float = 21600.0
    chat_timeout_recovery_enabled: bool = False
    chat_timeout_recovery_max_seconds: float = 900.0
    chat_timeout_recovery_poll_seconds: float = 2.0
    chat_auto_continue_enabled: bool = False
    chat_auto_continue_max_turns: int = 0
    ...
    def from_env(cls, opencode_url: str | None = None) -> "Settings":
        ...
        return cls(
            ...
            chat_timeout_recovery_enabled=_env_bool("EFP_CHAT_TIMEOUT_RECOVERY_ENABLED", False),
            ...
            chat_auto_continue_enabled=_env_bool("EFP_CHAT_AUTO_CONTINUE_ENABLED", False),
            chat_auto_continue_max_turns=max(0, int(os.getenv("EFP_CHAT_AUTO_CONTINUE_MAX_TURNS", "0"))),

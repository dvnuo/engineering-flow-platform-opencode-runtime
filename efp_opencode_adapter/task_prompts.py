...
def build_task_prompt(*, task_id: str, task_type: str, input_payload: dict[str, Any], metadata: dict[str, Any], source: str | None = None, shared_context_ref: str | None = None, context_ref: Any = None) -> str:
    ...
    if task_type in {"github_review_task", "github_pr_review"}:
        return prompt + (
            ...
            "Do not write back to GitHub unless Portal policy explicitly allows mutation tools.\n"
    ...
    if task_type in {"jira_workflow_review_task", "jira_workflow_review"}:
        return prompt + (
            ...
            "Do not transition or reassign Jira issues unless Portal policy explicitly allows the specific mutation action.\n"
            "If required Jira issue data is unavailable, return status=blocked and list missing fields.\n"
        )
    if task_type == "delegation_task":
        return prompt + (
            ...
            'Final output must include output_payload.delegation_result as object with fields status, summary, artifacts, blockers, next_recommendation, audit_trace.\n'
        )
